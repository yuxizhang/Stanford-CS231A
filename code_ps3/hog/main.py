...
def compute_gradient(im):
    ...
    for i in range(1, H - 1):
        for j in range(1, W - 1):
            ...
            p4 = im[i, j - 1]
            p6 = im[i, j + 1]
            angles[i - 1, j - 1] = atan_wrapper(np.arctan2(p2 - p8, p4 - p6))
            mag[i - 1, j - 1] = np.sqrt((p4 - p6) ** 2 + (p2 - p8) ** 2)

    return angles, mag

def atan_wrapper(degrees):
...
def generate_histogram(angles, magnitudes, nbins = 9):
    ...
    for i in range(M):
        for j in range(N):
            ang = angles[i, j]
            mag = magnitudes[i, j]
            bin1, bin2, diff1, diff2 = find_two_closest_center(ang, centers)
            hist[bin1] += mag * abs(diff2) / bin_size
            hist[bin2] += mag * abs(diff1) / bin_size

    return hist

def find_two_closest_center(ang, centers):
    diff = abs(centers - ang)
    # Update diff for first and last bin
    if (180 - centers[-1] + ang) < diff[-1]:
        diff[-1] = 180 - centers[-1] + ang
    if (180 - ang + centers[0]) < diff[0]:
        diff[0] = (180 - ang + centers[0]) 
    bin1, bin2 = np.argsort(diff)[0:2]
    return bin1, bin2, diff[bin1], diff[bin2]
...
def compute_hog_features(im, pixels_in_cell, cells_in_block, nbins):
    ang, mag = compute_gradient(im)
    H, W = ang.shape

    pixels_in_block = pixels_in_cell * cells_in_block
    stride = pixels_in_block / 2
    H_blocks = (H - pixels_in_block) / stride + 1
    W_blocks = (W - pixels_in_block) / stride + 1

    hog = np.zeros((H_blocks, W_blocks, cells_in_block ** 2 * nbins))

    for bi in range(H_blocks):
        for bj in range(W_blocks):
            bx1 = bi * stride
            bx2 = bx1 + pixels_in_block
            by1 = bj * stride
            by2 = by1 + pixels_in_block
            block_ang = ang[bx1 : bx2, by1 : by2]
            block_mag = mag[bx1 : bx2, by1 : by2]
            block_hog = np.zeros((cells_in_block, cells_in_block, nbins))

            for ci in range(cells_in_block):
                for cj in range(cells_in_block):
                    cx1 = ci * pixels_in_cell
                    cx2 = cx1 + pixels_in_cell
                    cy1 = cj * pixels_in_cell
                    cy2 = cy1 + pixels_in_cell
                    cell_ang = block_ang[cx1 : cx2, cy1 : cy2]
                    cell_mag = block_mag[cx1 : cx2, cy1 : cy2]

                    block_hog[ci, cj, :] = generate_histogram(cell_ang, cell_mag, nbins)

            block_hog = np.reshape(block_hog, -1)
            hog[bi, bj, :] = block_hog / np.linalg.norm(block_hog)
...
if __name__ == '__main__':
    ...
    im = np.array([[1, 2, 2, 4, 8],
                    ...
                    [1, 1, 1.5, 2, 2.5]])
    grad_angle, grad_magnitude = compute_gradient(im)
    print "My angles: ", grad_angle
    print "My magnitude: ", grad_magnitude
